...
/// The edited fields returned by [showSubtaskEditorSheet].
class SubtaskDraft {
  /// Wraps the validated, trimmed [title] and optional [notes].
  const SubtaskDraft({required this.title, this.notes});
...
/// Shows the subtask editor as a modal bottom sheet, pre-filled from [subtask].
///
/// Returns the edited [SubtaskDraft] on save, or null if dismissed. Save is
/// disabled while the title is empty or over-length (see: [titleError]).
Future<SubtaskDraft?> showSubtaskEditorSheet(
  BuildContext context, {
  required Subtask subtask,
}) {
  return showModalBottomSheet<SubtaskDraft>(
    context: context,
    isScrollControlled: true,
    // Root navigator so the barrier covers the shell, matching the task editor.
    useRootNavigator: true,
    builder: (context) => _SubtaskEditorSheet(subtask: subtask),
  );
}

class _SubtaskEditorSheet extends StatefulWidget {
  const _SubtaskEditorSheet({required this.subtask});
